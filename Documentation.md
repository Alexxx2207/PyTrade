...
Използва се PostgresSQL релационна база данни. Backend API-то е уеб сървър на Python с Flask WSGI. Клиентът е браузър, които е получил статични файлове като HTML, CSS, JS от Vite development web server.
...
Таблиците на базата данни се създават с миграции чрез ORM-a SQLAlchemy и SQL querybuilder-a Alembic. Тоест на Python пишем класове, които после се интерпретират от SQLAlchemy и се създват миграции(пак Python код), които после се използват за генериране на SQL заявки от Alembic.
...
Новогенерираната цена се запазва в базата данни и се праща на "събсрайбъри" по websocket протокол за текущата цена(за това по-надолу).
...
Комуникацията между сървър и клиент се случва по HTTP. Заявките се приемат от webserver(които ако е development webserver, е Wergzeug). Flask e web framework, който помага с четенето на HTTP заявката по мрежата и парсването ѝ, като я трансформира до Python "примитиви", които са лесни за използване. Примерно трябва да се вземат хедърите, path параметрите или query параметрите. Или да се рутира заявка с подаден път до python функция, която да се изпълни.

Важно: Flask не определя многонишковостта. Видът webserver, който може да ползва WSGI-а Flask, определя това. Wergzeug създава нишка за всяка заявка. Пример за друг уебсъвър, които обаче работи със threadpool с фиксиран размер е Waitress и той би се ползвал на production.
...
## Пращането на новогенерираната цена на актив в реално време

Изпращането на цената в реално време на определени клиенти изисква поддържането на мрежова връзка между сървъра и клиента. Такава връзка се създава и поддържа с транспортния протокол WebSocket, който изисква да се направи handshake между сървъра и клиента по HTTP и после да се комуникира по TCP.

За целта се изпозва библиотеката `socket.io`, която се базира на този протокол. Ако уебсокети не се поддържат от браузъра, се преминава към стандарните long/short polling.

Когато клиентът отиде на определена страница, се инициира "websocket" връзка. Тоест започва се обмяна на HTTP заявки и отговори с цел да се установи връзка между клиента и сървъра и после да може клиентът да се абонира за промяна на котировката на актива, a след това сървърът да му праща текущата цена. От страната на сървъра, той имплицитно държи списък със сокет дескриптори, които да ползва за пращане на новата цена, когато тя се генерира. В модула за генериране на цена се извиква callback функция, която уведомява със socketio.emit() абонати за даден финансов актив.

Самите абонати се разделят на групи спрямо това за кой актив искат да "слушат". Така че при промяна на цената на даден актив не се уведомяват всички "събскрайбъри", а само тези, които са в съответната група. Цялата логика по приемане на абонати, за тяхното групиране, за изпращането на пакети и за подтвърждаване за тяхното пристигане, се грижи `socket.io` библиотеката.
...
Hurst експонентата е индикатор за дългосрочна зависимост (long-range dependence) в редица от случайни величини:

> H ≈ 0.5: поведение близко до случайна разходка (без памет)

> H > 0.5: тренд/моментум

> H < 0.5: mean-reversion, т.е. често се мени посоката

Важно: в тази версия не се работи върху ценовите нива, а върху разликите между последователни цени.
...
После се намира кумулативните отклонения от средната:
...
После намираме стандартно отклонение:

![alt text](image-7.png)

Накрая делим двете величини (Rescaled range): `𝑅 / S`

Тоест, за конкретен прозорец `w` се взима средната стойност на `R/S`.

### Оценка на Hurst експонента

За прозорец `w` се предполага зависимост:
...
Следователно `H` се оценява като наклона на линейна регресия върху точките:
...
Функцията `logspace_intervals(min_w, max_w, count)` генерира списък от размери на прозореца `w`, разпределени логаритмично между min_window и max_window. Това осигурява да не се взимат прекалено много близки стойности. Ако се взимат с линейно отстояние, за големите стойности ще имаме много близки точки, тъй като логаритмуваме, а логаритъмът е вдлъбната функция.

### Паралелизация

Изчислението на средноаритметичното `R/S` за различните прозорци `w` е независимо, затова се паралелизира:
...
Пермутационна ентропия(PE) е базирана на времева редица от 1-минутни цени на затваряне. PE измерва "непредсказуемостта" на редицата чрез честотите на различни рангови шаблони на нейни подредици.
...
- `m` — дължина на шаблона / брой елементи на подредицата

- `tau` — стъпка между елементите на подредицата
...
За всяка подредица се построява пермутация, която описва реда на подреждане на стойностите (ранговете).

Подреждаме по стойност, а при равенства — по индекс k, за да има детерминиран резултат.

### Броене на честоти на шаблони

Изчислява се честотата на всеки от `m!` шаблона върху цялата редица. Това става чрез предварително построена таблица perm_to_id, която "мапва" пермутация с индекс [0..m!-1].

После се броят честотите в масив `counts[id] += 1`.

### Как се определя броят валидни стартови индекси

За да е дефинирана подредица с m и tau, трябва да има достатъчно бъдещи точки.
...
Това last_start се използва и като общ брой подредици при ентропията.

### Изчисляване на ентропия на Шанон

От честотите се построяват вероятности:

![alt text](image-8.png)

където N е броят на всички участващи подредици.

Тези вероятности после се ползват за Шанонова ентропия:
...
Нормализирана ентропия е `Hnorm = H / Hmax` и тя попада в [0,1].
...
Ползват се процеси, защото поради мютекса GIL(Global Interpreter Lock) нишките не се изпълняват паралелно, а конкурентно.
...
Целта е да има няколко задачи на процес за по-балансирано натоварване на процесите.

#### 9.3. Сумиране на резултатите

Всеки процес връща частичен `counts` масив. Накрая се сумират частичните броячи в един общ `counts`.

### Параметри на публичен API

minute_series: входна редица от цени на затваряне.

m: дължина на подредицата (по подразбиране 3).

tau: отстояние между елементите на подредицата (по подразбиране 1).
...
### Изисквания и препоръки за параметрите
...
Към проекта е написан допълнителен сървър с цел упражнение на selector с threadpool архитектурата. Ползва се еднонишков selector(демултиплексор) на "задачи", който може да изчаква множество сокети/файлови дескриптори и да указва кои от тях са готови за четене или за писане, без активно изчакване (busy-waiting). Когато даден сокет е готов за четене или писане, операционната система събужда тази нишка, ако тя е заспала(вследствие на липса на работа). Тя може да бъде събудена и ако изтече предварително конфигуриран таймер.

Архитектурата комбинира selector с threadpool от фиксиран брой работни нишки. Когато се прочете заявка от сокет и се извика submit(), задачата се поставя в опашка на ThreadPoolExecutor. Ако всички нишки са заети, задачите чакат в тази опашка до освобождаване на нишка. От страна на ОС новите входящи TCP връзки, ако не могат да се приемат веднага, поради заетост, чакат в listen backlog до accept().

А ако дадена нишка вече работи със създадена връзка, но не бива изпълнявана, докато се получават данни, то новодошлите байтове се пазят в буфера на TCP връзката, докато не бъдат прочетени с recv().
...
Трябва да се създаде база данни с това име и с права за писане и четене от потребител, който ще бъде ползван за заявките. В .env файл трябва да се добави DATABASE_URL. Примерен файл е .env.example в server папката.

## Прилагане на миграциите

Трябва да се приложат създадените миграции от SQLAlchemy в server/alembic/versions

В папката sever трябва да се изпълни командата:
...
Трябва да се стартира сървър локално на порт 5000.
